...
savePath = os.path.join(videoFolder, videoName)
...
if evalCol == True:
	...
	# ```
	videoName = 'col'
	videoFolder = colSavePath
	savePath = os.path.join(videoFolder, videoName)
	videoPath = os.path.join(videoFolder, videoName + '.mp4')
	duration = 0
	if os.path.isfile(videoPath) == False:  # Download video
		link = 'https://www.youtube.com/watch?v=6GzxbrO0DHM&t=2s'
		#cmd = "youtube-dl -f best -o %s '%s'"%(videoPath, link)
		cmd = "yt-dlp -f best -o %s '%s'"%(videoPath, link)
		output = subprocess.call(cmd, shell=True, stdout=None)
	if os.path.isdir(videoFolder + '/col_labels') == False: # Download label
		link = "1Tto5JBt6NsEOLFRWzyZEeV6kCCddc6wv"
		cmd = "gdown --id %s -O %s"%(link, videoFolder + '/col_labels.tar.gz')
		subprocess.call(cmd, shell=True, stdout=None)
		cmd = "tar -xzvf %s -C %s"%(videoFolder + '/col_labels.tar.gz', videoFolder)
		subprocess.call(cmd, shell=True, stdout=None)
		os.remove(videoFolder + '/col_labels.tar.gz')	
else:
	videoPath = glob.glob(os.path.join(videoFolder, videoName + '.*'))[0]
	savePath = os.path.join(videoFolder, videoName)
...
def evaluate_network(files):
	...
	for file in tqdm.tqdm(files, total = len(files)):
		...
		audio, samplingRate = librosa.load(os.path.join(pycropPath, fileName + '.wav'), sr=16000)
...
def visualization(tracks, labels):
	global videoName
	global videoFolder
	global pretrainModel
	global nDataLoaderThread
...
def evaluate_col_ASD(tracks, scores):
	global videoName
	global videoFolder
	global pretrainModel
	global nDataLoaderThread
	...
	txtPath = videoFolder + '/col_labels/fusion/*.txt' # Load labels
	predictionSet = {}
	...
	flist = glob.glob(os.path.join(pyframesPath, '*.jpg')) # Load files
	flist.sort()
	faces = [[] for i in range(len(flist))]
	for tidx, track in enumerate(tracks):
		score = scores[tidx]				
...
def speechToText(audioFilePath2):
...
# Main function
def main(fileName="front"):
	sys.stderr.write(time.strftime("%Y-%m-%d %H:%M:%S") + " Start \r\n")
	global videoName
	global videoFolder
	global pretrainModel
	global nDataLoaderThread
	...
	#If duration did not set, extract the whole video, otherwise extract the video from 'start' to 'start + duration'
	if duration == 0:
		command = ("ffmpeg -y -i %s -qscale:v 2 -threads %d -async 1 -r 25 %s -loglevel panic" % \
			(videoPath, nDataLoaderThread, videoFilePath))
	else:
		command = ("ffmpeg -y -i %s -qscale:v 2 -threads %d -ss %.3f -to %.3f -async 1 -r 25 %s -loglevel panic" % \
			(videoPath, nDataLoaderThread, start, start + duration, videoFilePath))
	...
	# Speech to text
	speechThread = threading.Thread(target = speechToText, args = (audioFilePath,))
	# captions = speechToText(audioFilePath)
	speechThread.start()
	...
	files.sort()
	final_labels = evaluate_network(files)
	savePath = os.path.join(pyworkPath, 'final_labels.pckl')
	with open(savePath, 'wb') as fil:
	...
	faces = visualization(vidTracks, final_labels)
